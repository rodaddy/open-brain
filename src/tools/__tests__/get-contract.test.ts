...
describe("get_contract", () => {
  ...
  it("advertises hot memory only through the agent_context_pack pull boundary (#271)", async () => {
    ...
    try {
      ...
      // push/injection surface was added.
      //
      // The capability entry above is ALSO 3. #691 originally moved only the
      // tool contract and this comment said the capability "stays at 2
      // deliberately" -- that was the drift, recorded as intent. The Python
      // client requires both blocks at 3 (client.py:78) and refused every
      // direct capture while they disagreed. The two must move together;
      // src/contract.test.ts asserts they agree for every tool.
