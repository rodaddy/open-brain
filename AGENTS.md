...
- **Embeddings:** Any OpenAI-compatible endpoint via `EMBEDDING_BASE_URL`. Prod and the local clone use `embed-gemma-dense` (768-dim, matches the stored vector space) on the fleet llama-swap: k3s `ai/llama-swap`, reached at `https://llama-swap.rodaddy.live/v1`, RTX 5060 Ti. The local MLX embedding server is retired (2026-08-25, #757); `EMBEDDING_WATCHDOG_RESTART_SCRIPT` stays empty because there is no local daemon to bounce.
