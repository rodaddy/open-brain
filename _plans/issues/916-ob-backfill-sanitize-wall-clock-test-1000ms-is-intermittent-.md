...
# #916 — ob-backfill sanitize wall-clock test (1000ms) is intermittent on the shared runner

State: CLOSED
Author: rodaddy
Labels: none
Created: 2026-08-27T08:26:49Z
Updated: 2026-08-27T17:13:19Z
Closed: 2026-08-27T17:13:19Z

---

scripts/ob-backfill.test.ts:255-259 asserts the token-dense scan finishes under 1000ms by wall clock. On #914's db-integration run (33053849804) it took 1024ms on the self-hosted runner while the file was untouched; main at 248bc199 passed the same test. A wall-clock threshold on a shared runner is a distribution tail, the same class as #912. Rule 42 handling applied: compared with main, rerun once, filed here. Fix belongs at the test: assert growth shape (two sizes, ratio) or a generous threshold, not 1s absolute. Filed by Claude (head, session 8).
...
### rodaddy — 2026-08-27T17:11:27Z

Session 9: PR #934 rewrites both sanitize scan assertions (token-dense and URL-dense) to growth shape: time(4N) / Math.max(time(N), 1) under 8, content assertions kept. New check scripts/done-means/916-sanitize-scan-growth-shape.sh: RED exit 1 at origin/main (clause 1a named the wall-clock assertions at :259 and :271, clause 1b found 0 ratio assertions), GREEN exit 0 on the branch. Measured on the Mac: 252.7ms at 12000 tokens, 972.5ms at 48000, quotient 3.85. Also closes #834 (same assertion). Collector running.
