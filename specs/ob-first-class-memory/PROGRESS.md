...
- Took a read-only PostgreSQL 18 custom-format logical dump from deployment_host and restored it into a uniquely named local database. The dump and local clone were treated as sensitive: restrictive permissions, structural/count-only inspection, no content output, and cleanup after evidence capture.
...
- Corrected deployment language against `.github/workflows/ci.yml` and `scripts/deployment_host-deploy-local.sh`: the gated deploy proves primary health and then runs a Bun regression test, so a real hosted changed-tool MCP smoke remains a separate post-workflow gate.
