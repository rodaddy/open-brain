...
- **Look before you leap.** Before writing an implementation of anything,
  ...
  a fleet-standard library, the standard library. Search first, write second.
...
- Do not write your own logger, HTTP client, retry/backoff, date/time math,
  ...
  production, not in review. Examples already used in this fleet — TypeScript:
  `pino`, `zod`, `undici`; Python: `loguru`, `pydantic`, `httpx`, `tenacity`.
  Examples, not an allowlist: prefer whatever the fleet already uses for the
  same job.
...
- When you do add a library, look before you leap there too: maintained, typed,
  and ideally already used elsewhere in the fleet. Do not trade a hand-rolled
