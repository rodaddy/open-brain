# Handover — #780 lint sweep, one file per lane, feeding L2 (2026-08-26)
...
- Layer 0.1: read open-brain/_DOCS/HANDOVER-RULES.md in full (147 lines). It
...
is `main` or `docs/handover-780-sweep` is merged (PR #TBD), switch first, never work there.
Retire: `docs/handover-rules-19-21` (PR #767), `fix/no-local-inference-defaults`
...
Commit this handover: branch `docs/780-sweep-paperwork`, path `_DOCS/_handover/2026-08-26-lint-sweep-l2.md` plus `_DOCS/HANDOVER-RULES.md` (rules 22-26), explicit-path
staging, `git commit -F` message file. The authoring session opened this as a PR on `docs/780-sweep-paperwork` (done-means `scripts/done-means/handover-validates.sh`); merge it in State 3 with the others, then cut a fresh `docs/780-sweep-paperwork-2` for this session's own paperwork.
...
Scope: PRs #782 (`chore/780-lint-main-ts`), #781 (`docs/ladder-status-l2a`), #783 (this handover); `docs/lane-contract.md` for the harvest
...
Must NOT: touch `search-engine.ts` (next handover: its L4 split), `process.env` reads, `src/`
Record: the PR, then tick on #780
...
- `search-engine.ts` (9 findings, `max-lines`) and `langfuse-tracing.ts` (8, `max-lines`) are the next handover: each is its L4 split pulled forward, split along `_plans/463-server-rewrite-charter.md` seams.
