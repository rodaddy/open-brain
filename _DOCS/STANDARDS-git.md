...
- **After a merge, pull fresh `main` and branch from there for the next task.**
  Every task starts from the merged state:

  ```bash
  git checkout main && git pull        # or: git fetch origin
  git checkout -b <type>/<slice>       # from the tip that now includes the merge
  ```
