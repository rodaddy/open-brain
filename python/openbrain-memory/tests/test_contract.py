...
def test_validate_contract_manifest_reports_min_client_version_failure():
    ...
    assert result.reasons == (
        ...
        f"{safe_string_display('>=0.1.5 <1.0.0')}",
    )
...
def test_validate_contract_manifest_rejects_prerelease_client_version():
    ...
    result = validate_contract_manifest(manifest, client_version="0.1.5-alpha")

    assert result.ok is False
    assert result.reasons == (
        "client_version '0.1.5-alpha' is not a supported semver version",
    )
