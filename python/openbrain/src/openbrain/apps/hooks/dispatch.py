...
Pattern/Convention:
    ...
    ``Stop``, ``SubagentStop``, ``SessionEnd``, and ``PostCompact`` run real
    work -- the two captures deliver through the spine, the close releases the
    server slot, and ``PostCompact`` records the compaction summary the Stop
    spine drops; the rest are stubs that drain stdin and exit 0. The distinction
    is in the modules, not here -- this file treats them identically, which is
...
#: ``SessionEnd`` releases the server slot, and ``PostCompact`` records the
#: compaction summary; every other value is a stub. The names match
#: ``hook_event_name`` exactly, as the harness sends it.
