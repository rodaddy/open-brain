...
const OUT = "/path/to/open-brain/_tmp/open-brain/_scratch/gateport-fixtures";
const SCRIPTS = "/path/to/open-brain/Development/_ob/scripts";
mkdirSync(OUT, { recursive: true });
const scratch = mkdtempSync(join("/path/to/open-brain/_tmp/open-brain/_scratch", "gaterec-"));
...
const cwd = "/path/to/open-brain/Development";
const probes: Probe[] = [
  // --- context-budget-gate ---
  ...
  { name: "policy-blocked-risky-write", script: "policy", argv: policyArgs("pre-tool-use", "pol-2"), stdin: { session_id: "pol-2", cwd, tool_name: "Write", tool_input: { file_path: "/path/to/open-brain/Development/x.md" } } },
  { name: "policy-blocked-allows-refresh-cmd", script: "policy", argv: policyArgs("pre-tool-use", "pol-2"), stdin: { session_id: "pol-2", cwd, tool_name: "Bash", tool_input: { command: "bun /path/to/open-brain/Development/_ob/scripts/policy-refresh-gate.ts --event refresh --agent claude" } } },
