...
/**
 * Everything above tests `parseServerConfig` with a hand-built environment.
 * `loadServerConfig` is the thin wrapper that reads the real `process.env` and
 * feeds it to that same reader, so the environment SHAPE a launchd service
 * hands the process — the shape that crashed it — is what these exercise, via
 * `parseServerConfig` directly rather than by mutating the live process.
 */
describe("loadServerConfig-shaped environment through parseServerConfig", () => {
  function parseFrom(overrides: Record<string, string | undefined>) {
  ...
  it("still throws a named, field-identifying error on a genuinely invalid environment", () => {
    ...
    expect(() => parseFrom({ DB_HOST: "" })).toThrow(
