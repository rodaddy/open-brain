 ...
 */
export function resolveFtsConfig(language: string | null | undefined): FtsConfig {
 ...
 */
export function corpusFtsConfig(env: NodeJS.ProcessEnv): FtsConfig {
 ...
 */
export function ftsStatementTimeoutMs(env: NodeJS.ProcessEnv): number {
...
/**
 * The env-free call form the legacy retrieval modules use (issue 864, rule
 * M13a). server/ code may not read process.env, and the two call sites inside
 * legacy-search hold no server config, so the environment arrives through a
 * registered READER instead. The default reader returns an empty environment,
 * which yields exactly the documented defaults above.
 */
export type FtsEnvReader = () => NodeJS.ProcessEnv;

const defaultFtsEnvReader: FtsEnvReader = () => ({});

let ftsEnvReader: FtsEnvReader = defaultFtsEnvReader;

/** Install the reader the env-free selectors read from; returns the previous. */
export function setFtsEnvReader(read: FtsEnvReader): FtsEnvReader {
  const previous = ftsEnvReader;
  ftsEnvReader = read;
  return previous;
}

/** `requestFtsConfig` over the registered environment reader. */
export function requestFtsConfigFromReader(
  requested: string | null | undefined,
): FtsConfig {
  return requestFtsConfig(requested, ftsEnvReader());
}

/** `ftsStatementTimeoutMs` over the registered environment reader. */
export function ftsStatementTimeoutMsFromReader(): number {
  return ftsStatementTimeoutMs(ftsEnvReader());
}
