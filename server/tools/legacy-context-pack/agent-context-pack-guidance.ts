...
/**
 * Select the promoted rows that survive supersession and per-scope-key dedupe,
 * in the order the query returned them (promoted-most-recent first, then id).
 */
function selectPromotedRows(
  normalized: LifecycleRow[],
  candidateType: string,
): LifecycleRow[] {
  const retired = retiredScopeKeys(normalized);
  const seenScopeKeys = new Set<string>();
  const selected: LifecycleRow[] = [];
  for (const row of normalized) {
    ...
    if (retired.has(row.scopeKey)) continue;
...
/**
 * Turn the selected rows into guidance items and their matching citations,
 * reporting whether any item was omitted or shortened by a caller-supplied
 * allowance.
 */
function collectGuidanceItems(
  ...
  const { candidateType, maxItems, maxItemChars } = options;
  const items: Array<Record<string, unknown>> = [];
  const citations: Array<Record<string, unknown>> = [];
  let itemsTruncated = false;

  for (const row of selectPromotedRows(normalized, candidateType)) {
    ...
    if (!bounded.text) {
      // Non-empty content the char allowance cannot admit is an omission,
...
export async function loadGuidanceSection(
  ...
  try {
    ...
    const collected = collectGuidanceItems(normalized, {
      candidateType,
      maxItems,
      maxItemChars,
    });
    const { items, citations, itemsTruncated } = collected;
