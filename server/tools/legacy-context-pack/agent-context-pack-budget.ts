 ...
 */
export function sectionFrameCost(key: string, isFirstAdmitted: boolean): number {
...
/**
 * Find the longest prefix of the lane checkpoint text whose rebuilt section
 * still serializes within `remainingChars`, by binary search. Returns null when
 * there is no text to keep or no prefix fits.
 */
function fitCheckpointText(
  contextMd: unknown,
  remainingChars: number,
  rebuild: (text: string) => Record<string, unknown>,
): unknown {
  ...
  while (low < high) {
    ...
    const fits = serializedLength(rebuild(contextMd.slice(0, mid)));
    if (fits <= remainingChars) low = mid;
    else high = mid - 1;
  }
...
export function fitDurableLaneSection(
  ...
  let contextMd: unknown = lane?.current_context_md ?? null;
  ...
  // it entirely.
  contextMd = fitCheckpointText(contextMd, remainingChars, (text) => rebuild([], text));
...
export function reconcileDurableCitations(
  ...
  return citations.filter((citation) => {
    if (citation.kind === "session_event") {
      return typeof citation.id === "string" && keptEventCitationIds.has(citation.id);
    }
    return true;
  });
}
 ...
 */
export function durableMemoryContentChars(section: DurableMemorySection): number {
  let total = 0;
...
export function reconcileDurableMemoryCitations(
  ...
  return citations.filter(
    (citation) => typeof citation.id === "string" && keptCitationIds.has(citation.id),
  );
}
