...
function ingestResult(counts: IngestCounts) {
  return {
    content: [
      {
        type: "text" as const,
...
/**
 * Drop harness scaffolding before anything touches the database, and redact
 * every surviving turn. Returns the turns worth writing plus how many were
 * dropped as noise.
 */
function partitionTurns(turns: RawTurnInput[]): {
  kept: PreparedTurn[];
  filtered: number;
} {
...
const INSERT_COLS = 19;

/** Provenance and threading columns of one turn. */
function lineageValues(ns: string, t: RawTurnInput): unknown[] {
  return [
    ns,
    t.turn_uuid,
...
/** Payload and telemetry columns of one prepared turn. */
function payloadValues(clientId: string, prepared: PreparedTurn): unknown[] {
...
/** Column values for one prepared turn, in the INSERT column order. */
function turnValues(ns: string, clientId: string, prepared: PreparedTurn): unknown[] {
  return [...lineageValues(ns, prepared.turn), ...payloadValues(clientId, prepared)];
}

/**
 * One multi-row INSERT: the whole batch is one round trip, and
 * ON CONFLICT DO NOTHING makes a replayed batch a no-op rather than an error
...
async function insertTurns(
  ...
  for (const [index, prepared] of kept.entries()) {
    ...
    values.push(...turnValues(ns, clientId, prepared));
  }
...
/**
 * Assign session_seq server-side (migration 036). The client's turn_index
 * cannot be trusted for ordering: it is a per-invocation counter, so every
 * session in the corpus carried only 0..7 regardless of length. A client cannot
 * fix this -- it sees one batch, retries, resumes, and several runtimes write
 * the same session -- so the server owns the invariant its own schema declares.
 *
 * Recomputed for the whole affected session rather than appended to. Batches
 * arrive out of order (spool replay, retries), so a late turn must be able to
 * land in the MIDDLE of the sequence; appending would order by arrival instead
 * of by when the turn happened. The (session_ref, occurred_at) pair is unique
 * across the live corpus, and id breaks any tie deterministically.
 *
 * Failure here is logged, not fatal: the turn is already durably stored, and a
 * NULL session_seq is repairable by re-running this statement. Losing captured
 * conversation to a numbering error would be the far worse outcome.
 *
 * Affected sessions are derived from the whole validated `kept` payload, NOT
 * only from the INSERT's RETURNING rows. If a prior call stored the turns but
 * its seq recompute failed transiently, those rows carry session_seq = NULL; a
 * replay of the same turn_uuids returns zero RETURNING rows (ON CONFLICT DO
 * NOTHING), so keying off RETURNING alone would skip the recompute and the
 * NULLs would never repair. Keying off the payload's session_refs makes an
 * ordinary replay the repair path the comment above promises. The recompute is
 * idempotent (`session_seq IS DISTINCT FROM o.seq` updates only rows that
 * change), so re-running it for an already-numbered session is a no-op.
 *
 * Scoped to `ns`. session_ref is client-supplied and is NOT unique across
 * namespaces -- the only uniqueness on the table is (namespace, turn_uuid)
 * (migration 032). Two namespaces can carry the same session_ref, so a
 * recompute partitioned by session_ref alone would renumber another
 * namespace's rows by interleaving this write's occurred_at values into their
 * ordering. Adding `namespace = $2` to both the ordering window's source and
 * the UPDATE keeps each namespace's session_seq computed only from its own
 * turns, which is what the per-namespace isolation boundary requires
 * (AGENTS.md: any ID-based mutation carries an auth-derived namespace
 * predicate).
 */
async function recomputeSessionSeq(
  pool: ToolDeps["pool"],
  ns: string,
  kept: PreparedTurn[],
): Promise<void> {
  ...
  if (touchedSessions.length === 0) return;
...
/** Resolve the authorized namespace, or the denial to return to the caller. */
function authorizeIngest(
  auth: AuthInfo | undefined,
  requested: string | undefined,
): { ns: string } | { denied: ReturnType<typeof ingestError> } {
...
async function ingestBatch({ deps, clientId, ns, kept, filtered }: IngestBatchArgs) {
  try {
    const ingested = await insertTurns(deps.pool, ns, clientId, kept);
    const duplicates = kept.length - ingested;

    await recomputeSessionSeq(deps.pool, ns, kept);
    ...
    return ingestResult({ ingested, duplicates, filtered, namespace: ns });
  } catch (err) {
...
export function registerIngestRawTurn(server: McpServer, deps: ToolDeps): void {
  server.registerTool(
    "ingest_raw_turn",
    {
      ...
      inputSchema: {
        namespace: z
          ...
          .max(500)
          .optional()
          .describe("Namespace for isolation (defaults to the caller's clientId)"),
        turns: z
          .array(rawTurnSchema)
          .min(1)
          .max(MAX_BATCH)
    ...
    async (args, extra) => {
      ...
      const authorized = authorizeIngest(auth, args.namespace);
      if ("denied" in authorized) return authorized.denied;
      const { ns } = authorized;

      const { kept, filtered } = partitionTurns(args.turns);
      ...
      return ingestBatch({
        deps,
        clientId: (auth as AuthInfo).clientId,
        ns,
        kept,
        filtered,
      });
