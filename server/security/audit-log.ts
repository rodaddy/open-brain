...
/**
 * The env values `readMcpAuditConfig` needs, supplied by the caller rather
 * than read from `process.env`: server/ owns no environment access, so the
 * composition root passes what it already parsed into `ServerConfig.mcpAudit`.
 */
export interface McpAuditEnvironment {
  readonly [key: string]: string | undefined;
  readonly OPENBRAIN_MCP_AUDIT_ENABLED?: string | undefined;
  readonly OPENBRAIN_MCP_AUDIT_RETENTION_DAYS?: string | undefined;
  readonly OPENBRAIN_MCP_AUDIT_CLEANUP_INTERVAL_MS?: string | undefined;
  readonly OPENBRAIN_MCP_AUDIT_WRITE_TIMEOUT_MS?: string | undefined;
}

export interface McpAuditDeps {
  pool: pg.Pool;
  /**
   * Absent means audit is off for this server. Before the move this fell back
   * to `readMcpAuditConfig()` reading `process.env`; server/ reads no
   * environment, so the composition root passes `ServerConfig.mcpAudit` and a
   * caller that supplies nothing gets the disabled path rather than an
   * env-derived one.
   */
...
export function captureRawArgsForAudit(rawArgs: unknown, parsedData: unknown): void {
  if (!rawArgsCaptureEnabled) return;
  if (!parsedData || typeof parsedData !== "object") return;
  rawArgsCaptureByParsed.set(parsedData as object, captureArgsFacts(rawArgs));
}

function takeRawArgsAuditCapture(parsedArgs: unknown): RawArgsAuditCapture | undefined {
...
export function readMcpAuditConfig(env: McpAuditEnvironment): McpAuditConfig {
  return {
...
// Ascending upper bounds, each paired with the label a payload at or below it
// gets. A table rather than an if-ladder so the bucket edges read as data and
// the lookup carries one decision point instead of ten.
const PAYLOAD_SIZE_BUCKETS: ReadonlyArray<readonly [number, string]> = [
  [0, "0b"],
  [128, "le_128b"],
  [512, "le_512b"],
  [1024, "le_1kb"],
...
export function payloadSizeBucket(input: unknown): string {
  ...
  const bucket = PAYLOAD_SIZE_BUCKETS.find(([max]) => bytes <= max);
  return bucket ? bucket[1] : "gt_1mb";
}
...
function estimateJsonBytes(input: unknown): number | null {
  ...
  while (stack.length > 0) {
    ...
    // Null means unserializable (a cycle, or a BigInt anywhere), which the
    // caller buckets as "unknown".
    const bytes =
      typeof value === "object"
        ? visitContainer(value as object, seen, stack, total)
        : estimateScalarBytes(value);
...
export function summarizeMcpAudit(input: {
  ...
  return {
    ...
    durationMs: Number.isFinite(input.durationMs)
      ...
      : 0,
    ...callerIdentityFields(input.auth),
...
export function installMcpAudit(server: McpServer, deps: McpAuditDeps): void {
  ...
  if (typeof validatorHost.validateToolInput === "function") {
    ...
    validatorHost.validateToolInput = async (tool, args, toolName) => {
      ...
      try {
        ...
        await writeMcpAuditWithTimeout(
          deps,
          config,
          state,
          summarizeMcpAudit({
  ...
  server.registerTool = ((name: string, configOrDescription: unknown, cb?: unknown) => {
    ...
    const declaredKeys = declaredParameterKeys(
      (configOrDescription as { inputSchema?: unknown } | undefined)?.inputSchema,
    );
...
async function writeMcpAuditWithTimeout(
  ...
  const waitingCount = (deps.pool as { waitingCount?: number }).waitingCount ?? 0;
  if (state.inFlightWrites >= MAX_IN_FLIGHT_AUDIT_WRITES || waitingCount > 0) {
  ...
  const outcome = await Promise.race([write.then(() => "written" as const), timed]);
...
// Brackets/braces plus one comma per member. Counted before the members
// themselves are pushed so the estimator's early exit can fire on width alone.
function containerStructuralBytes(value: object): number {
  const memberCount = Array.isArray(value)
    ? value.length
    : Object.keys(value as Record<string, unknown>).length;
  return 2 + memberCount;
}

// Pushes each member onto the traversal stack and returns the bytes owed by
// the object keys (quotes, colon, comma); array members owe no key bytes.
function pushContainerMembers(value: object, stack: unknown[]): number {
  ...
  for (const [key, item] of Object.entries(value as Record<string, unknown>)) {
    keyBytes += key.length + 3;
    stack.push(item);
  }
  return keyBytes;
}

// Approximate JSON bytes for a non-object, non-null value. Null means the
// value is a BigInt, which JSON.stringify refuses anywhere in a payload.
// Nested function/symbol contributes nothing: JSON omits the member (object)
// or emits null (array), and either way the contribution is negligible.
function estimateScalarBytes(value: unknown): number | null {
  switch (typeof value) {
    case "string":
...
// Accounts for one container: its structural bytes, plus the key bytes of any
// members it pushes onto the traversal stack. Members are pushed only when the
// running total is still under the early-exit boundary, so an adversarially
// wide container is measured on width alone and never walked. Null means a
// cycle, which JSON.stringify cannot serialize.
function visitContainer(
  ...
  seen.add(value);
  const structural = containerStructuralBytes(value);
  if (runningTotal + structural > SIZE_ESTIMATE_EARLY_EXIT_BYTES) {
    return structural;
  }
  return structural + pushContainerMembers(value, stack);
}

// The caller-identity columns of an audit row. Every field is nullable by
// construction: the validation-failure path runs before any auth context
// exists, so "no caller" is a real and expected state rather than a defect.
function callerIdentityFields(
  ...
  | "namespaceSource"
> {
  // One optional-chain at the top rather than one per field: `AuthInfo`'s own
  // members are already `string | undefined`, so a single nullish coalesce per
  // column is all the normalization each one needs.
  const present: Partial<AuthInfo> = auth ?? {};
  return {
    callerRole: present.role ?? null,
    callerClientId: present.clientId ?? null,
    callerTokenClientId: present.tokenClientId ?? null,
    callerAgentId: present.agentId ?? null,
    namespaceSource: present.namespaceSource ?? null,
  };
}
