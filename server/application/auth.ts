...
import type { AuthInfo, Role } from "../../src/types.ts";
...
export function authMiddleware(
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (matched) {
      ...
      const rejection = rejectDelegationHeaders(matched, namespace, agentId);
      if (rejection) {
...
/** An Express request carrying the auth identity this middleware attaches. */
type AuthenticatedRequest = Request & { auth: AuthInfo };

/**
 * Validate the delegation headers against the matched identity. Returns the
 * refusal to send, or undefined when the headers are acceptable.
 */
function rejectDelegationHeaders(
  matched: AuthInfo,
