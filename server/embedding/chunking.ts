...
/**
 * Choose where a chunk that does NOT reach the end of the text should stop.
 *
 * Prefers sentence-ending punctuation followed by whitespace and an uppercase
 * letter inside the overlap zone; falls back to the last word boundary, and
 * finally to the caller's proposed `end` when neither is usable. Hoisted out of
 * chunkText so each function stays a single decision; behavior is unchanged.
 */
function findBreakPoint(text: string, window: BreakPointWindow): number {
  const { start, end, chunkSize, overlap } = window;
  ...
  if (sentenceBreak !== -1) {
    return zoneStart + sentenceBreak + 1;
  }
  // Fall back to last word boundary
  const lastSpace = text.lastIndexOf(" ", end);
  return lastSpace > start + chunkSize / 2 ? lastSpace : end;
}
...
export function chunkText(
  ...
  while (start < text.length) {
    ...
    if (end < text.length) {
      end = findBreakPoint(text, { start, end, chunkSize, overlap });
    }
