 ...
 */
export function createRotatingFileSink(options: RotatingFileOptions): RotatingFileSink {
  const path = options.path;
  const maxBytes =
    options.maxBytes && options.maxBytes > 0 ? options.maxBytes : DEFAULT_MAX_BYTES;
  const maxFiles =
