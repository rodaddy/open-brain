...
function normalizeField(values: unknown): string[] {
  ...
  for (const raw of values) {
    ...
    const bounded =
      trimmed.length > MAX_ITEM_LENGTH ? trimmed.slice(0, MAX_ITEM_LENGTH) : trimmed;
...
function normalizeExtraction(raw: unknown, text: string): ExtractedMetadata | null {
 ...
 * clobbered.
 */
export function extractedTagCandidates(extracted: ExtractedMetadata | null): string[] {
  if (!extracted) return [];
  const seen = new Set<string>();
  const out: string[] = [];
...
/**
 * The row background extraction enriches, plus the text it reads. Grouping the
 * per-row fields into one object keeps the function to three parameters.
 */
export interface BackgroundExtractTarget {
  entryId: string;
  namespace: string;
  text: string;
  existingTags?: string[];
 ...
 * candidates, so no current tag can be lost. `existingTags` on the target is
 * accepted for caller symmetry only; it is not used to compute the result.
 */
export function backgroundExtract(
  pool: pg.Pool,
  table: string,
  target: BackgroundExtractTarget,
): void {
  const { entryId, namespace, text } = target;
