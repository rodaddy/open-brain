...
| 29 | Decisions pass 2026-08-08 (evening): live-eval env, launcher keys, bulk-heal | **resolved** | With the operator, one item each. (1) **Live-eval credentials file AUTHORIZED** — the controller assembles `~/.local/share/openbrain-memory/env/live-eval.env` (mode 600, never in git) from values already on this machine (observation env + local-clone.env), enabling the #653 E2E gate's credentialed verify. Rejected: operator hand-builds it (slower, same values, no added safety since the values already live on this box). (2) **All six announced-dropped launcher keys get honored** — EMBEDDING_WATCHDOG_RESTART_SCRIPT, LOG_LEVEL, LOG_MAX_BYTES, LOG_MAX_FILES, OPENBRAIN_MCP_AUDIT_ENABLED, SERVICE_NAME join CHILD_ENV_KEYS via a fast lane; they were configured deliberately, so dropping them is the bug (round-18 allowlist class). Rejected: honor-all-but-watchdog (splits config semantics between dev and prod for no measured reason); leave-dropped-and-trim-env (throws away deliberate config to silence a correct announcement). **AMENDED same evening (29.2a), operator-ruled after the lane held at RED:** honor FIVE, not six. The lane proved the six-key premise wrong for `EMBEDDING_WATCHDOG_RESTART_SCRIPT`: it is set EMPTY in the env file — the documented clone-mode SUPPRESSION form (docs/local-clone-dogfood.md:147) — and sits on `PROHIBITED_PATH_KEYS` (src/local-clone-mode.ts:15, test-pinned), so honoring it would contradict the clone-isolation guard; it appeared in the announcement only because the drop reporter skips `undefined` but not empty-string. Ruling: honor the five valued keys, keep the watchdog suppressed, and fix the reporter to distinguish unset / set-empty / set-valued. Rejected at amendment: honor-five-leave-reporter (the false-positive class recurs for every empty-suppression); hold-everything (five keys were correctly ruled and are only blocked by the sixth's misclassification). Process note: a ruling enumerating items from a log inherits that log filter's classification errors — the lane's hold-at-RED-and-escalate is the no-variations rule working as designed. (3) **Bulk-heal for the 2,011 pre-#650 lanes stays LAZY** (the recorded default) — each heals on next touch, zero migration risk. Rejected: one-shot bulk migration (only pays off if dormant lanes need captures soon, which nothing shows). |
| 30 | Decisions pass 2026-08-08 (late): eval delegation ownership + lane-bootstrap continuation | **resolved** | With the operator. (1) **The E2E scenario transport owns its delegation request** — `eval/open-brain/live/scenario-transport.ts` sets `OPENBRAIN_DELEGATE_NAMESPACE=1` when spawning the provider, because the component that derives per-run foreign namespaces is the one that always needs the header; intent lives in code with its reason. Found by the second credentialed #653 verify: the post-#657 opt-in default OFF correctly refused the gate's derived namespace, and the #662 error text named the exact remedy — two fixes proving themselves in one failure. Rejected: flag in live-eval.env (buries intent in machine-local operator config; every future credential assembly must remember it); both (redundant). (2) **lane-bootstrap gains a continuation mode** for existing branches (worktree without `-b`, verify branch matches origin, same .env/install steps, red-first check) — hand-building is what ledger 15 exists to stop, and round 21 recorded the hand-build only as an interim. Note item 15's teardown rejection stands untouched: continuation mode is bootstrap-side only, teardown stays manual. Rejected: document-the-hand-build-forever (keeps a manual replication of a scripted path as standing procedure). |
| 31 | Database DELETE of proven test data is not the filesystem no-delete rule | **resolved** | Operator ruling 2026-08-08: "if it's test data and you know for sure it's test data, just remove it. It's a database, not an RM-RF." Executed same minute: 104 rows under `eval-live-recall-%` (73 thoughts, 30 decisions, 1 lane) deleted in one transaction, children first, verified zero remain. SCOPE of the ruling: a prefix-scoped SQL DELETE of rows the agent can PROVE are its own test residue (guarded prefix, counted before and after) is the agent's to run — the unconditional no-`rm` rule governs the filesystem, not this. What still is NOT covered: rows of uncertain provenance (the `eval-kwdiag-1889c167` row stays untouched — different eval family, not proven junk), user-owned data, and anything outside a provably-test prefix. Prior posture (report SQL for the operator's hand) was the safe default before this ruling existed; lanes may now clean their own proven test rows and must announce counts. |
| 32 | Issue mirrors are ARTIFACTS: a completed node must show its outcome and why | **resolved** | Operator ruling 2026-08-09, verbatim: "the mirrors are not mirrors. They are artifacts that need to exist locally and we should treat them as such when completed, we should show that they are completed. And so when we do an AQMD search, they're known to be completed, and they explain the direction we went in and why we went in them. That is the whole point of this new version of graph execution/coding." THE GAP: `scripts/sync-issues.ts` mirrored every issue body and comment (its header's "COMMENTS ARE THE POINT" premise, unchanged since 2026-07-28) but rendered a closed issue as `State: CLOSED` plus a timestamp — while the RESOLUTION lives in the closing PR: its body (which in this repo carries done-means RED/GREEN evidence and the critical self-review), its merge commit, and its merge time. A search therefore found the question and not the answer. WHAT LANDED: a DELTA on the existing generator — closed issues now render a `## Resolution` section naming the closing PR, its merge SHA, merged-at, and its body reproduced BYTE-FOR-BYTE (blockquoted so the PR's own `##` headings cannot masquerade as artifact sections); no parallel artifact system, one generated file per issue, GENERATED header and overwrite-on-sync semantics all preserved. 263 of 263 closed issues render a Resolution. MEASURED TRAP that decided the implementation: `closedByPullRequestsReferences` returns EMPTY here — lanes squash-merge into a wip branch rather than the default branch, so GitHub's auto-close linkage never registers, and the field reports `[]` for #681 even though PR #687 (squash `31589d1`) closed it. Building on it would have asserted ABSENCE on exactly the issues that have a resolution. The timeline carries it in three measured shapes over the 25 most recent closures: closer is a PullRequest (20), closer is a Commit whose SHA matches a cross-referenced PR's mergeCommit (2), closer is null (3, closed by hand — renders the close actor and state reason and names no PR). PROCESS HALF, because the trail is only as good as what agents write: the controller posts a CLOSURE COMMENT on the ISSUE at merge (direction + why + receipts), added as controller-contract obligation 2b and lane-contract round 27. REJECTED OPTIONS: (a) a parallel "outcomes" artifact system — forks the searchable surface in two and abandons the mirror's own no-hand-edit guarantee, when the gap is one missing section on an existing generator; (b) render the cross-referenced PR list as "possibly related" — #659 is cross-referenced by four PRs and closed by one, so it would put several directions on an issue that took one and leave the reader unable to tell which, which fails the ruling as surely as rendering nothing; (c) rely on the closure comment alone without the generator change — leaves 263 already-closed issues with no resolution and makes the trail depend entirely on future discipline; (d) hand-write resolutions into the mirror files — they are overwritten on every sync by design. Two lane-caught defects worth the ledger: importing the generator to test it FIRED THE WHOLE SWEEP and rewrote `_plans/issues/` (a check that mutates what it measures cannot be trusted about it — side effects now guarded by `import.meta.main`), and the driver's derived body-phrase expectation was fabricated by word-filtering, failing a CORRECT renderer and reading exactly like a renderer defect (derived expectations now self-check that they exist in the source). |
| 33 | Manual wrap ADOPTS a hook-owned lane's scope; the client-side hostile-same-namespace denial test is RETIRED | **resolved** | Operator rulings 2026-08-17 (two, same day): (1) wrap adopts the lane's existing scope rather than refusing or asserting its own (the #724 item-4 fix, PR #732); (2) when the lane proved the client emits IDENTICAL wire sequences for "the capture hook's lane" and "another same-namespace session's lane" — so adoption and the old `test_checkpoint_first_lane_denies_hostile_same_namespace_claim` cannot both hold — the ruling was retire the test and merge. Rationale: same-namespace is same-tenant; the enforced boundary is the server's namespace predicate; AGENTS.md Coding Standards already declare client-side convenience checks are not security controls, and the retired test asserted a distinction the client provably cannot observe. Rejected: (a) server-side adoptable-lane marking first (real boundary, but delays the wrap fix for a distinction inside one tenant); (b) refuse-loudly fallback (leaves manual wrap permanently dead for no security gain). Any future cross-session lane-claim hardening is server work under a new issue. |
...
| `tracking-scribe` | every merge pass since 2026-08-07, re-sent by hand each time as the paste-in prompt "TRACKING SCRIBE run for..." | standing run (`sync-issues.ts` mirror refresh, `stale-blockers.ts` candidate report, commit `_plans/issues` by explicit path, `aqmd up`) + the harvest of lanes' returned learnings into `docs/lane-contract.md` Tightenings, `docs/sme/entries/`, and this ledger; report-and-mirror only — never closes, never merges | **built** — 2026-08-10, operator scope decision (do NOT widen `pr-scribe`; separate agent). `.claude/agents/tracking-scribe.md`, gated by `scripts/done-means/tracking-scribe-root-only.sh`. Its FIRST law is ROOT-ONLY writes with the `aqmd`-visibility WHY stated in the definition — the paste-in form kept dropping that guardrail, which is how 164 lines of harvest rounds were stranded in worktrees (`_plans/worklog/reconcile-root-2026-08-10.md`). `docs/sop-rlvr-lanes.md` step 5 and `docs/controller-contract.md` obligation 2 now NAME it as the harvest owner instead of describing an inline controller action |
