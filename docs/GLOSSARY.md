...
| **Extracted metadata** | Structured JSON stored in `extracted_metadata` on `thoughts` and `decisions`. Deterministic runtime write-time extraction is enabled (`src/extraction.ts`, `backgroundExtract`): every new row gets a zero-network structural pass emitting `title`, `content_hash`, `hash_version`, `byte_length`, plus deterministic ISO `dates`; the semantic fields (`topics`, `people`, `action_items`) are contributed only by an injected metadata provider and default to empty. All values are content-free (a bounded title, an opaque digest, an algorithm tag, a length, and normalized/capped labels) — never the raw source body. | The raw content or user-supplied tags. |
