...
`scripts/dev/check_code_size.py`, because ruff has no line-count rule at all**
...
via `scripts/dev/demo-hooks.sh`, which asserts each violation is rejected *by
the named check*:
