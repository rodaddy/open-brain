...
| deploy | deployment_host macOS runner | manual/tag | production rollout |
...
- `core.hooksPath` is set to
  `/path/to/open-brain/Development/open-brain/.git/hooks` — the *untracked*
  directory.
...
| `deploy` | `[self-hosted, macOS, deployment_host]` | production rollout; needs all five above |
...
- `workflow_dispatch` with `deploy_deployment_host: true` **on `main`**
- a push of a `v*` tag

Runs on `[self-hosted, macOS, deployment_host]` with
`concurrency: { group: deploy-deployment_host-production, cancel-in-progress: false }` —
...
| deployment_host | `self-hosted, macOS, deployment_host` | production deploy only |
