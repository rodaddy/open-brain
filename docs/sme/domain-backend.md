...
A bounded context query must fetch one extra row before truncating so omission flags are truthful, use a total order such as `(created_at, id)` in both selection and returned chronology, and keep timestamp comparison database-owned. PostgreSQL ordering boundaries must not round-trip through millisecond-precision `Date` values, which can collapse distinct timestamps and skip or duplicate rows. Enforce latency inside the database operation: `Promise.race` alone leaves a detached query consuming a pool connection; use a checked-out client plus statement timeout/cancellation and clean rollback before release. Tests must cover sub-millisecond timestamps, timestamp ties, exactly-at-limit versus over-limit results, and a concretely wedged query.
