...
| Runtime reflex read (#371) | Enforced client-side: `FirstClassMemoryRuntime.reflex` routes the published `agent_reflex_pointers` tool through `_runtime_router` as a pure read — **direct-only, never spooled and never mcp2cli-fallback** (a read must not queue or read through another path). Before any dispatch the fresh live `get_contract` proof must publish `agent_reflex_pointers` at its published semantic version (`FIRST_CLASS_RUNTIME_TOOL_VERSIONS` in `client.py`); a v23 manifest that omits it or advertises the wrong/malformed version yields zero reflex dispatches and a FAILED content-free receipt. Input carries the exact scope coordinates plus a required bounded `query` and an optional body-free `prior_context` (only `citation_id`/`source_ref` identities; raw bodies and extra fields are rejected before the transport). The server result is never surfaced raw: `project_reflex_result` validates the returned exact scope, the ordinary `openbrain.agent_reflex_pointers.v1` envelope (`schema`, `status`, `placement=client_owned`, `resolvable_reference_only=true`, `scope`, `pointers`, `warnings`, `budget`, `citations`, `query`), the body-free pointer items (identity/`source_ref`/structural metadata only, unknown/body/display fields rejected), the `label=pointers`/`namespace_scoped`/`item_count`/`truncated` pointer invariants, and the `kind=pointer` citation `citation_id` bijection, then rebuilds a fresh safe mapping through explicit content-free allowlists — the untrusted original is never mutated or returned. The receipt is content-free (`durable=false`, no pointer bodies) and a reflex failure surfaces a **stable category label** (`reflex_request_invalid` / `reflex_result_invalid` / `reflex_dispatch_failed`), never redacted exception text | `python/openbrain-memory/src/openbrain_memory/runtime.py`, `_runtime_router.py`, `_runtime_validation.py`, `client.py`, `cli.py`; regressions in `tests/test_runtime_reflex.py`, `tests/test_contract.py` |
