...
| Runtime reflex read (#371) | Enforced client-side: `FirstClassMemoryRuntime.reflex` routes the published `agent_reflex_pointers` tool through `_runtime_router` as a pure read — **direct-only, never spooled and never mcp2cli-fallback** (a read must not queue or read through another path). Input carries the exact scope coordinates plus a required bounded `query` and an optional body-free `prior_context` (only `citation_id`/`source_ref` identities; raw bodies and extra fields are rejected before the transport). The result is surfaced only after proving the returned exact scope and the ordinary `openbrain.agent_reflex_pointers.v1` envelope; the receipt is content-free (`durable=false`, no pointer bodies) | `python/openbrain-memory/src/openbrain_memory/runtime.py`, `_runtime_router.py`, `_runtime_validation.py`, `cli.py`; regressions in `tests/test_runtime_reflex.py` |
