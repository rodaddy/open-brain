...
### 2026-08-29 (round 44) — harvest of the session-14 issue-864 L5 move lanes (PRs #990, #992 through #1005), which cut `src/` out of the `server/` runtime closure

- **Choose shim versus adapter from the CALLERS and from the module's own TESTS, not from the module.** `src/shared-namespace.ts` had seven `server/` importers and not one held a `ServerConfig` at its call site, so the "retarget the importers" half of the brief resolved to zero edits and the lane collapsed to two files; `src/drop-folder-collector.ts` was env-free itself while `src/drop-folder-collector.test.ts` set `DROP_COLLECTOR_MAX_FILE_BYTES` inside individual `it` bodies, so a shim would have compiled, typechecked, and silently broken those assertions. `rg -n "process\.env" <module> <module>.test.ts` before the `git mv`, and treat a hit in the TEST file as decisive on its own. (PRs #995, #998)
- **A `server/` module sharing a `src/` module's basename is not its shim, and `head -1` is the whole check.** Four of seven dependencies in one lane were shims and repointing them was free; three were distinct implementations whose contract already required injected config, and repointing those compiled cleanly, passed lint, and stopped honoring `OPENBRAIN_PROMOTION_KILL_SWITCH` while turning every scan and promote into a non-JSON 500. An `// L5 shim` or `// L5 adapter` header means the repoint is safe; its absence means keep the `src/` import with a comment saying why. (PR #997)
- **A `ServerConfig` group shaped like the parameter a moved module needs can still differ in absent-versus-blank handling, and the types match perfectly on both sides.** `resolveQmdPath` branches on `QMD_PATH === undefined` and passes a blank value through untrimmed; `qmdGroup` is a trimmed blank-as-absent field, so wiring the module to `config.qmd.path` changes what `QMD_PATH=""` resolves to with every test and typecheck green. `server/config/src-readers.ts` had already written the warning in a comment. Compare the group's builder against the code being moved for absent-versus-blank, trimming, and falsy-versus-`undefined` before reusing it. (PR #996)
- **What selects a lazy reader over a snapshot is not where the env read sits but whether anything can register before the value is consumed.** M13a named per-call reads inside function bodies; this module read env at MODULE SCOPE in all nine places and still needed the reader, because a module-scope read runs during import, strictly earlier than any importer's chance to register. Keep it a single read and defer it to first USE (memoize in an accessor), and let the default reader return the env-absent defaults so an unregistered `server/` import behaves identically to the unconfigured original rather than failing or inheriting someone else's settings. (PR #1000)
- **A shim satisfies importers and silently breaks consumers that read the module BY PATH as text.** `scripts/done-means/681-tool-role-liveness.driver.ts` `readFileSync`s the ingest module and regex-matches its role enum; after the move the shim held neither, so a clause flipped PASS to FAIL while every import resolved and `tsc` and the moved tests stayed green. The same class made the contracts parity scanner report a falling tool count that read as normal drift. Survey with `rg -n "<basename>" scripts server src python --type ts --type py` BEFORE the first `git mv`, reading for `readFileSync`/`Bun.file`/glob lists rather than imports, and require the identical clause verdict from the affected driver before and after. (PRs #993, #999)
- **The done-means check is the only gate that reads dependency DIRECTION, type-only imports included; run it before the suite, not after.** `src/tools/search-brain.ts` passed typecheck, lint, and the whole suite while clause A failed on one line, `import type { Tier } from "../types.ts"`. The fix is to derive the type from one the adapter already imports from `server/` (`type Tier = NonNullable<ExecuteSearchOptions["tier"]>`), not to move it. Run the script with NO arguments: the argument form only judges the paths you name and passes a bad draft. (PRs #1005, #995)
- **A twin adapter is found by exported SYMBOL across `server/`, and its log event names and helper symbols are diffed against the old tests before anything is repointed.** A log-record filter with no assertion after it passes just as green when a rename makes it match zero records, so every filter is paired with a non-empty assertion. A done-means driver that compared the `src/` side against the `server/` side is rewritten in the same commit to assert the two imports resolve to the SAME function; a comparison of a module against its own adapter is a self-comparison. (the context-pack lane, branch `refactor/864-c9d-context-pack-adapters`)
- **A dependency field declared and read but unfilled at the composition root is exit 0 on `tsc` and green on the suite.** `MemoryToolDependencies.dropCollectorBounds` was optional, so the handler fell back to defaults and all five `DROP_COLLECTOR_*` overrides were silently dropped. The proof is a non-default value carried end to end — a real three-level tree returning strictly fewer files under a configured depth of 1 than under the default 8, both arriving through `parseServerConfig` — never a shape assertion. (PR #998)
- **Whoever replaces a process-wide injected singleton owns restoring it, on normal shutdown and on failed start, and the setter must RETURN the previous value or the restoration cannot be written at all.** `server/main.ts` registered its config-backed reader and never put back the adapter's env reader, so a stopped server's config answered for everything after it; in a test process, "afterwards" is the next file. Neither file's own tests catch it — the evidence is a COMBINED run against the same combined run at `origin/main` (114/0 before, 112/2 after, 151/0 once `shutdown` restored). (PR #1002)
- **Inherited whole-file lint debt is measured on every path you will STAGE, tests included, before the move is planned — and it lands as its own pull request when it is independent of the lane's feature.** `contracts/check-parity.ts` was red at `main` at 529 code lines, so a one-line scanner fix could not be staged at all and split mechanically into its own PR from `origin/main`. A driver whose cheap read is "one import line" cost an M14 hoist of every pre-existing `as any` plus a prettier reformat, revealed only at `git commit` after tests and done-means had already run. Lint the intended staged set with `oxlint --deny-warnings --config .oxlintrc.json` BEFORE the suite. (PRs #999, #1004, #992)
- **A `server/` module's SIGNATURES are free to change; the legacy call form lives in the adapter.** Positional lists become one options object and big handlers become named helpers, while the adapter keeps every exported positional signature — a labelled rest tuple (`...tail: LegacyGraduateTail`) keeps call sites and argument names intact at one parameter when the list is wider than four. Behavior is proven by the existing tests passing through the adapter, not by signature identity, and a lane finding max-params or complexity failures writes the adapter instead of stopping. The count is a property of the moved file, not of the brief: `src/` is not linted under the `server/` overrides, so lint the moved file before assuming the brief enumerated the findings. (PR #992)
- **A closure-variable loop does not survive extraction into named helpers unless its state becomes one explicit object first.** `createNatsJsDriver`'s resubscribe loop shared four mutable flags across four inline blocks; extracting them would have passed each by value while the `close()` handle mutated the original, silently breaking backoff and the empty-subscription degrade. One `ResubscribeState` object passed by reference, with the loop's fixed collaborators as `readonly` fields, drops the parameter count to one. Trace each flag on the THROW path specifically — the old body left `processedMessage` false and skipped the degrade helper there, and no test in this repo drives that path. (PR #1003)
- **A splice-based hoist anchored on `export async function X(` lands between X's JSDoc and X, detaching the comment onto the wrong function or leaving a stray unterminated `/**`.** It happened three times in one lane, in all three refactored modules, and typechecked and linted clean every time. Anchor on the START of the doc block or the blank line above it, and re-read the four lines either side of every insertion point before running checks. (PR #1001)
- **When the pinned formatter floats ahead of what is committed, every lane touching an old file inherits its reflow and "change one import line" stops being achievable.** `server/main.ts` was prettier-clean when committed, but pinned prettier 3.9.6 reformats it, so the pre-commit gate REFUSED the minimal two-line diff M8 asks for and the runtime importer's diff is mostly whitespace. A lane hitting this says so in the PR body rather than letting the reflow read as unrequested scope. `prettier --write` on an untouched file is the mirror trap — it rewrote a file to defaults disagreeing with the committed formatting and dragged in 27 unrelated lines (restore with `git show HEAD:<path> >`; `git checkout --` is refused by the git guard). (PRs #990, #1004)
- **Check WHICH symbols `server/main.ts` actually imports before planning an options widening, and grep `server/config/` for the env keys before adding a reader.** One lane's entrypoint imported only `composeMaintenanceHandlers` and `MAINTENANCE_GRAPH_AUTH`, neither of which read env, so the whole env surface belonged to legacy `src/` callers and collapsed into the adapter with a one-line import change; `parseMaintenanceConfig` already parsed all six `OPEN_BRAIN_MAINTENANCE_*` values, so the new-reader allowance was unnecessary. Expect earlier lanes to have SPLIT a sibling — let `tsc` name the new home rather than assuming the old grouping survived a mechanical path rewrite. (PR #994)

provenance: PRs #990, #992, #993, #994, #995, #996, #997, #998, #999, #1000, #1001, #1002, #1003, #1004, #1005; issue #864; the session-14 lane reports and the head's M9-M16 rulings.

### 2026-08-29 (round 43) — harvest of the session-13 program close: the #962 allowance lane (PR #964), the 878-program-complete check lane (PR #963), and the #924 diagnosis lane

- **A done-means clause that greps a multi-token source line is written against the formatter's output, not the lane's typed layout.** The brief's clause 3 named the literal closer `}, GROWTH_SCAN_ALLOWANCE_MS);`; the pre-commit prettier pass expands a three-argument `it()` onto separate lines and deletes that exact string, which voids the checker itself and not merely a receipt. The lane matched the allowance as an `it()` argument in either layout and re-took RED, deliberate miss, and GREEN on the committed tree. (PR #964)
- **A committed done-means log path is config; it takes the `${OPENBRAIN_SCRATCH:-...}` shape the moment it is first typed.** The head's brief hard-wired `_scratch/session13/` into `878-program-complete.sh`; a session-named folder in a committed script outlives the session and nothing fails loudly when it does. The follow-up commit be57927 used the precedent at `scripts/done-means/433-recall-sees-session-events.sh:140`. (PR #963)
- **Number hits before stripping comment lines.** The first draft filtered comment lines out and then reconstructed `path:line` from the surviving stream, off by every comment line above the hit; `rg -n` first, then the `^[0-9]+:\s*(//|/\*|\*)` filter. Only the deliberate-miss receipt ever exercises that numbering, because a green run never reaches it. (PR #963)
- **A red check whose one rerun is spent gets a fix lane per rule 45, and the blocked PR is rebased onto the fix from a clean clone.** #961 sat red on the #962 growth-scan timeout after its rerun (7637 ms); the allowance lane merged first, then `git rebase origin/main` plus `git push --force-with-lease` from lane-11 gave #961 a check job that ran with the fix, and it merged green. A hand merge on a red check is never the answer (rule 52). (PRs #964, #961)
- **`aqmd in <repo>` from a lane clone scopes on the clone since #960 and tries to build a private index there.** Both write lanes lost their design lookup (`.qmd/index.yml is empty`); the Read-tool route on a `docs/decisions/*.md` file satisfies the design-lookup hook and is what a clone brief names until #965 is settled. (#965)
- **`mock.module` on a shared singleton is process-wide in bun and survives `mock.restore()`.** `scripts/__tests__/bulk-import.test.ts:17` replaces `src/logger.ts`, and `server/observability/langfuse-tracing.test.ts` then fails to load or never reaches `console.warn` (`src/logger.ts:539`); three sibling test files already carry comments forbidding that mock. The two-file pair reproduces in the root checkout and passes in a clone on the same tree; why the checkouts differ is still open. (#924)

provenance: PR #963; PR #964; PR #961; issues #878, #924, #962, #965; the session-13 lane reports.

### 2026-08-28 (round 42) — harvest of the session-12 #878 append-session-event split (PR #957, eight sequenced steps in one clone) and the expectDefined util lane (PR #956)

- **A file the pre-commit gate will refuse cannot be split across per-lane pull requests; sequence the steps in one clone and land one commit.** `_githooks/pre-commit` checks out the index and lints every staged file WHOLE, and a deleted path is not linted. The plan's Lanes 1-7 each staged the shrinking original with its ~130 pre-existing findings, so every one of those commits would have been refused until Lane 8 deleted the file. Eight step lanes ran in sequence on one branch with nothing committed, each verified on the working tree (oxlint on the new files, tsc, test:isolated per file, census falling, the 878 check), and step 8 committed nine files plus the deletion. The head recorded the deviation on #878 before the first step. (PR #957)
- **An uncommitted multi-step tree has no undo but the head's snapshot.** Two of seven step lanes reached for `git stash` or `git checkout HEAD -- <file>` to back out their own bad `sed`, and each reverted the original to origin/main, discarding every earlier step's deletions; both rebuilt it from saved extracts and proved the census. From step 6 the head copied the split files and a `git diff` of the original into `_scratch/session12/snap-step<N>/` after every step, and every later brief said: never stash, checkout, restore, or reset in this clone; derive the shortened file into scratch, diff, then `cp` it over. (PR #957)
- **The `CHANGED_FILES` form of `878-pg-tests-require-database.sh` judges a database demand, so it does not apply to a pure unit file, and the bare form reports `SUBJECT: none` while the pg file is untracked.** The plan's Lane 2-8 done-checks named the unit file and the original together and fail by construction (clauses 2-3 demand a `require*Url` call). The receipt for a unit step is `CHANGED_FILES=<the pg file>` plus `rg -n 'describe.skip|skipIf|dbDescribe|OPENBRAIN_TEST_DATABASE_URL'` over the split files with no code-line hit. (PR #957)
- **Every number in a brief is an anchor, including the manifest count.** The handover and the plan said `minTests 8` for `append_session_event create_if_missing (live Postgres)`; `scripts/assert-db-tests-ran.ts:40` carried 4 while JUnit emits 8, and rule 55's "entry exists, floor unchanged" reading sent step 1 the wrong way until the landing step corrected the entry and the floor by the measured delta. Four of seven plan groups were off by one or two its and the event-type loop array carries 9 entries, not 8; every lane moved what the file held and reported the real count. (PR #957)
- **A de-duplication lane diffs the bodies before it writes the util.** `rg "export function expectDefined"` reported nine identical-looking signatures; two threw a different message string that no test asserted. The first lane stopped on the divergence with receipts, the head ruled (the seven-copy message wins), and the second lane landed one util re-exported by nine helper modules. (PR #956, issue 951)
- **The repo text gate scans the Bash command string, so a heredoc carrying a hoist script is refused by the same words the detour was meant to avoid.** Write the script with the Write tool, then run it from Bash; test names never pass through Edit. A verbatim move plus hoisting can still exceed `max-lines`: shorten the hoisted names first, factor repeated mock rows, then reuse the helper module's `withToolClient`. On this machine `sed` is GNU, so an in-place range deletion is `sed -i '<range>d'`, and a deletion is verified by `wc -l`, never by the exit code. (PR #957)

provenance: PR #956; PR #957; issue #951; issue #878 session-12 comments; the session-12 lane reports.

### 2026-08-27 (round 41) — harvest of the session-11 #878 split-and-convert lanes (PRs #948, #949, #950), the #945 clause-2 lane (PR #947), and the append-session-event plan lane

- **An `as any` removal is finished at tsc, not at oxlint.** `any` suppresses resolution errors as well as type errors: this repo's `AuthInfo` and the MCP SDK's `AuthInfo` share a name and differ in shape, so twelve `auth: AuthInfo` annotations resolved to the SDK global once the casts went, failed tsc, and lint stayed clean. Alias the repo type on import, re-export it from the lane helper so both split halves annotate the same symbol, and cast only at the transport seam (`NonNullable<SendOptions>["authInfo"]`). `bunx tsc --noEmit` after each cast removal is the receipt. (PR #948)
- **A split lane takes every line range from a scratch copy of the origin/main file, never from the working copy being split.** The edit that creates the split invalidates every anchor; a range past EOF makes `sed -n` exit 0 with no output and yields syntactically valid empty function bodies that tsc accepts. `git checkout <path>` as a mid-lane recovery reverts to origin/main and discards every unrelated fix in that file. Extract once at the top, take every range from the copy, rebuild forward. (PR #950)
- **`MIN_TOTAL_LIVE_TESTCASES` is the sum of the manifest's `minTests`, so converting a suite that already has an entry raises it by zero.** Rounds 39-40 raised it only because each registered a NEW entry. Before touching the floor: is the describe name already in `REQUIRED_SUITES`? Present means change nothing; absent means add the entry at the JUnit count and raise by that count. `bun test scripts/assert-db-tests-ran.test.ts` is the one-second discriminator and belongs in the check list of any lane that edits that file. (PR #949)
- **Clause 2 of the #878 check now judges the demand: the import statement is joined from `import` to its `;` and one relay hop through a sibling module is resolved.** The round-40 workaround (one specifier per import line, each split half carrying its own call and import) is no longer required, though it remains the origin/main convention. A shell check that matched an identifier and a module specifier on one physical line was judging the formatter; where a check gates a SPLIT, the shared sibling helper module is exactly what a split produces. (PR #947, #945)
- **A lane that labels pushed-but-open pull-request files MERGED is over-claiming; the collector corrects it to WRITTEN.** The state word is the merge commit's, not the push's. (PR #949)
- **`node/no-process-env` under `server/**` is satisfied for a live test by moving the env set/restore into `scripts/test-support/`** (`shared-namespace-env.ts`: `isolateSharedNamespace`, `restoreSharedNamespace`), which keeps the behavior identical and the env access at the test-support seam. Whether the server's shared-namespace isolation should instead come through `parseServerConfig(environment)` is a Rico decision (HANDED-OVER UNKNOWNS).

provenance: PR #947; PR #948; PR #949; PR #950; issue #945; the session-11 lane reports.
### 2026-08-27 (round 40) — harvest of the session-10 #878 split-and-convert lanes (PRs #939-#942, #944), the #827 pin lane (PR #943), and the #938 diagnosis lane

- **A brief gives anchor lines; the lane takes block boundaries from the file.** The first embedding-repair lane consumed its whole timebox on `sed -n 'A,Bp'` copies whose ranges were transcribed from the brief and were off by one to six lines, and the corruption surfaced only as a parse error several appends later. Anchors name the `it(`, `describe(`, or `function` line; the lane prints the real start (doc comment) and end (`});`) with `awk 'NR>=A && NR<=B'` before every copy and checks each appended block with tsc before the next. A range that ends inside a describe body also carries the lifecycle hooks between two helpers. (PR #942)
- **A phase-N lane verifies the previous phase by the shape of the file being split FROM, not by the presence of staged paths.** `git status` showed every expected path staged and both new files lint-clean, so every file-existence signal said phase 2 was complete, while the original file still held eight its, a nested describe, and 794 lines. One `rg -n 'describe\(|^\s+it\('` plus `wc -l` on the source file is the discriminator: a real split makes both numbers fall. (pull request #944 (pushed, not merged))
- **Ungating a flag-gated suite: the flag-forced baseline run is the first receipt after RED.** `OPENBRAIN_BACKUP_DRILL=1 bun run test:isolated <file>` on unmodified origin/main showed 7 pass 1 fail in six seconds and exposed a defect no gate had ever seen (#938), before any lint or split work. A conversion brief for a suite gated on a non-database flag names that run first. (pull request #944 (pushed, not merged), #938)
- **A non-superuser restore target needs its extensions created by an administrator first.** Granting the clone role ownership of a fresh, empty database cleared restore's non-empty refusal and then failed on `pg_restore` creating `vector` and `pg_stat_statements`; the same bootstrap `scripts/test-support/clone-database.ts:188-194` performs for the runner's clone had to run in the drill's `openDrill`. (pull request #944 (pushed, not merged), #938)
- **The #878 clause 2 judges the import's line shape, not the demand.** A prettier-wrapped two-specifier import of the require helpers, and a split half that reaches the helper only through a shared lane module, both fail clause 2 while clauses 1, 3, and 4 pass. Until #945 lands: one named specifier per import line, and each split half carries its own `requireTestDatabaseUrl()` call and its own single-line import. (pull request #944 (pushed, not merged), #945)
- **Splitting one long `it` body into two sequential module-scope halves is safe only where the halves share no variable across the seam.** Check that independence before cutting; otherwise the split renames the `max-lines-per-function` finding instead of clearing it. (PR #939)
- **An `it`-body hoist takes its end line from the `});` that closes the `it`, never the line before the next `it`.** These bodies end in a multi-line `pool.query` whose `);` sits two lines from the close, so an end line one too high swallows a brace. Run prettier on a mechanically assembled file first: it localizes the splice error where tsc reports it far from the seam. (PR #940)
- **Under `noUncheckedIndexedAccess`, retyping a parsed payload with a recursive index signature lints clean and fails tsc on every nested read.** Enumerate the actual accesses (`rg -o 'payload\.[a-z_]+(\.[a-z_]+)*'`) and declare only those properties. (PR #941)
- **`max-lines-per-function` is reported at the describe callback and only becomes visible after a split lands; hoist every `it` body in one scripted pass up front.** The pre-commit gate's prettier rewrite invalidates every receipt taken before the commit: lint, tsc, tests, and the done-means are re-taken on the committed tree. (PR #942)
- **A done-means mutation clause anchors its post-mutation guard on the syntax of the statement it deletes, never on a bare identifier.** The source file's own prose repeats the identifiers its code uses, so a file-wide search for the event name reported a correct deletion as a failed mutation; verifying the mutation on a copy first showed it removed exactly the warn and left the adjacent `logger.error` intact. (PR #943)

provenance: PR #939; PR #940; PR #941; PR #942; PR #943; pull request #944 (pushed, not merged); issues #938, #945; the session-10 lane reports.
